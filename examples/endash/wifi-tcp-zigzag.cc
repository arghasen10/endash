...
//  apNodes.Get(0)->GetDevice(0)->GetAddress()
  ...
  if (firstLine)
    {
      return "nodeId,velo_x,velo_y,pos_x,pos_y,chId,#dev,Freq,ChNo,linkup,associated";
    }
...
#ifndef OLD_NS3
  stream << "," << std::to_string(mac->IsAssociated());
#else
  stream << ",";
#endif
...
void AssocDeassoc(bool assoc, Mac48Address val1) {
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("nAp", "The number of Ap in multiple-ue topology",
                conf.nAp);
  cmd.AddValue ("nSta", "The number of sta multiple-sta topology", conf.nAp);
...
//  NetDeviceContainer remoteHostDevice = csma.Install(remoteHostContainer);
  ...
  std::string trcSrc = "/NodeList/" + std::to_string(staNodes.Get(0)->GetId ()) + "/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::StaWifiMac/";
  Config::ConnectWithoutContext(trcSrc + "Assoc", MakeBoundCallback(AssocDeassoc, true));
  Config::ConnectWithoutContext(trcSrc + "DeAssoc", MakeBoundCallback(AssocDeassoc, false));
