...
void onStop(int *count){
	(*count) --;
	if(!(*count)) {
		std::cout<<"stopping at:" << Simulator::Now().GetSeconds() <<std::endl;
		Simulator::Stop();
	}
}
...
int main(int argc, char *argv[]) {
	...
	cmd.AddValue("fixedMcs",
			"The MCS that will be used in this example, -1 for auto", fixedMcs);
	...
	Ptr<ListPositionAllocator> apPositionAlloc = CreateObject<
			ListPositionAllocator>();
	Ptr<ListPositionAllocator> staPositionAlloc = CreateObject<
			ListPositionAllocator>();
	int32_t yValue = 0.0;
		...
		for (uint32_t i = 0; i < objectMapValue.GetN(); i++) {
			...
			if (i == 0) {
				bandwidthPart->GetPhy()->SetTxPower(
						10 * log10((bandwidthBwp1 / totalBandwidth) * x));
				std::cout << "\n txPower1 = "
						<< 10 * log10((bandwidthBwp1 / totalBandwidth) * x)
						<< std::endl;
			} else if (i == 1) {
				bandwidthPart->GetPhy()->SetTxPower(
						10 * log10((bandwidthBwp2 / totalBandwidth) * x));
				std::cout << "\n txPower2 = "
						<< 10 * log10((bandwidthBwp2 / totalBandwidth) * x)
						<< std::endl;
			} else {
...
//	Simulator::Stop(Seconds(simTime));
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i =
		...
		outFile << "  TxOffered:  "
				<< i->second.txBytes * 8.0 / (simTime - udpAppStartTime) / 1000
						/ 1000 << " Mbps\n";
