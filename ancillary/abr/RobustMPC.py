...
#==================
    ...
    if ( len(agentState.past_bandwidth_ests) > 0 ):
        curr_error  = abs(agentState.past_bandwidth_ests[-1]-past_bandwidths[-1])/float(agentState.past_bandwidth_ests[-1])
    agentState.past_errors.append(curr_error)
#==================
...
#==================
    ...
    max_error = 0
    error_pos = -5
    if ( len(agentState.past_errors) < 5 ):
        error_pos = -len(agentState.past_errors)
    max_error = float(max(agentState.past_errors[error_pos:]))
    future_bandwidth = harmonic_bandwidth/(1+max_error)
    agentState.past_bandwidth_ests.append(harmonic_bandwidth)
#==================
